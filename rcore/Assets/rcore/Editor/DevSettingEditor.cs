	...
	public class DevSettingEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			EditorHelper.BoxVertical(() =>
			{
				...
				{
					EditorUtility.SetDirty(m_profileCollections);
		...
		private void DrawSettingsProfile(DevSetting.Profile pProfile)
		{
			...
			if (pProfile.defines != null)
			{
				...
				if (EditorHelper.Button("Clone"))
				{
					...
					m_profileCollections.profiles.Add(cloneProfile);
				}
				if (m_previewingProfiles && EditorHelper.ButtonColor("Remove", Color.red))
					m_profileCollections.profiles.Remove(pProfile);
		...
		{
			var profiles = m_profileCollections.profiles;
			if (profiles.Count == 0)
				return;
		...
		private void DrawProfilesSelection()
		{
			EditorHelper.BoxVertical(() =>
			{
				var profiles = m_profileCollections.profiles ?? new List<DevSetting.Profile>();
